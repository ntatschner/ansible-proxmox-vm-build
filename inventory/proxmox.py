...
class ProxmoxAPI(object):
    ...
    def auth(self):
        ...
        data = requests.post(request_path, data=request_params,
    ...
    for node in proxmox_api.nodes().get_names():
        ...
        for vm in node_hostvars:
            ...
            try:
                ...
                type = 'qemu'
            try:
                description = proxmox_api.vm_description_by_type(node, vmid, type)[
                    'description']
            except KeyError:
                description = None
...
def main():
    ...
    parser.add_option(
        '--qemu_interface', default=os.environ.get('QEMU_INTERFACE'), dest='qemu_interface')
    parser.add_option(
